...
def get_date_from(text: str) -> datetime:
    ...
    raise ValueError("Wrong date format. Use dd/mm/yyyy or yyyy-mm-dd.")


def estimate_processing_time(uploaded_file_path: str, existing_file_path: str) -> str:
    size_uploaded = os.path.getsize(uploaded_file_path) / (1024 * 1024)
    size_existing = os.path.getsize(existing_file_path) / (1024 * 1024)
    total_size_mb = size_uploaded + size_existing

    processing_rate = 8.72

    estimated_time = total_size_mb * processing_rate
    ...
    if hours > 0:
        return f"Tempo estimado: {hours} horas, {minutes} minutos e {seconds} segundos."
    elif minutes > 0:
        return f"Tempo estimado: {minutes} minutos e {seconds} segundos."
    else:
        return f"Tempo estimado: {seconds} segundos."
